  ...
  # ==================== FULL-SCREEN QUIZ MODE ====================
    if st.session_state.get('taking_quiz', False) or st.session_state.get('show_results', False):
        ...
        if st.session_state.get('taking_quiz', False):
                ...
                with col2:
                    if st.button("✅ Submit Quiz", type="primary", use_container_width=True):
                        if len(st.session_state.quiz_answers) < len(quiz_questions):
                            ...
                            details = []  # ✅ ADDED: per-question review data
                            for i, question in enumerate(quiz_questions, 1):
                                ...
                                # ✅ ADDED: store details for review screen
                                details.append({
                                    "num": i,
            ...
            # ✅ ADDED: Show which questions were right/wrong + correct answer
            st.subheader("📝 Review (Correct / Wrong)")
            details = results.get("details", [])
            show_wrong_only = st.checkbox("Show only wrong questions", value=False)

            if not details:
                st.info("No question details available for review.")
            else:
                for d in details:
                    if show_wrong_only and d["is_correct"]:
                        continue

                    icon = "✅" if d["is_correct"] else "❌"
                    with st.expander(f"{icon} Question {d['num']}"):
                        st.markdown(f"**{d['question']}**")

                        # show options and mark
                        for letter, text in d["options"].items():
                            if not text:
                                continue
                            tags = []
                            if letter == d["correct_answer"]:
                                tags.append("✅ correct")
                            if letter == d["user_answer"]:
                                tags.append("👈 your choice")
                            tag_text = f" ({', '.join(tags)})" if tags else ""
                            st.write(f"- **{letter}.** {text}{tag_text}")

                        st.markdown(f"**Your answer:** `{d['user_answer']}`")
                        st.markdown(f"**Correct answer:** `{d['correct_answer']}`")

                        if d.get("explanation"):
                            st.caption(f"Explanation: {d['explanation']}")
